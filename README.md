...
By default, the independent axis is assumed to be redshift *z*. You can optionally configure it to *ζ* = ln(1+*z*) using the `--units zeta` flag across commands.

Once the PDFs are stored in the **coldpress** format, **coldpress** can perform multiple tasks with them, including resampling to a new grid, combination (conflation, averaging, correlation), visualization, and measurement of statistics (mode, mean, confidence intervals, etc.).
...
You can interact with **coldpress** via its Python API or its command-line interface (CLI) for processing FITS tables.

For a comprehensive guide detailing all CLI commands, required arguments, mutually exclusive flags, and operational constraints, please consult the **[ColdPress CLI User Manual](docs/cli_manual.md)**.

To quickly view the main help message and available commands in your terminal, run:
...
This section demonstrates a typical workflow using the `coldpress` command-line tool. We will inspect a FITS table, compress the PDFs, measure key statistics, and plot the results.

> [!IMPORTANT] The CLI of **coldpress** only operates on FITS tables. If your PDFs are stored in a different file format you need to convert to FITS first.
...
```
The `PDF` column contains the probability density *P(z)* sampled at 701 redshift values. To find the redshift for each, we inspect the FITS header:
...
> [!IMPORTANT]
> The `--density PDF` option tells **coldpress** that the `PDF` column contains P(z) sampled in a grid. If your column contains integrated probability in bins, or random samples, you must use `--binned` or `--samples` instead.
...
> [!TIP]
> Since version 1.2.0, you can plot multiple PDFs for the same source in a single plot by providing multiple column names to the `--encoded` argument.

You can also overplot any numerical quantity from the FITS table (such as the ones we calculated with `measure`, or even spectroscopic redshifts) as a vertical line using the `--quantities` flag followed by the relevant column names:
...
For example, to reconstruct the PDFs as a density grid with twice the redshift resolution (*z* steps of 0.005 instead of 0.1) using the highly recommended monotonic spline interpolation from *z*=0 to *z*=7 and stored in a column named PDF_HR:

```bash
coldpress decode hsc_sample_encoded.fits hsc_sample_decoded.fits --density PDF_HR --zmin 0 --zmax 7 --nvalues 1401 --method spline
```
> [!WARNING]
