...
[[NOTE: all measurements are performed using linear interpolation of the CDF between consecutive quantiles]]

[[NOTE: the random seed ensures reproducibility 
of Z_RANDOM values at the table level. Two identical PDFs in the same table will obtain different random values. Also, the Z_RANDOM values for the same PDF with change if the order of the PDFs in the table is altered.]]

[[NOTE: The odds parameter represents the integrated probability in a window centered at the  corresponding point estimate (i.e, Z_MEDIAN for ODDS_MEDIAN, Z_MODE for ODDS_MODE) of half-width equal to ODDS_WINDOW*(1+z).]]
...
[[NOTE: be careful with the --plot-all option on a large table!]]

[[NOTE: use the interactive mode for close inspection of the PDFs. It allows to pan and zoom.]]

[[NOTE: the quantities that can be plotted as vertical lines are not restricted to the ones generated by the coldpress measure command. Any scalar value can be shown. It is particularly useful for diagnostics to show the spectroscopic redshift if available.]]
...
[[NOTE: this is a healthy practice to identify issues before compression.]]

[[NOTE: --density is not implemented yet, but users can use --binned to check density-type PDFs.]]

[[NOTE: currently there are no checks on whether the binned or sampled PDFs extend beyond the redshift range that can be encoded with ColdPress. This will be fixed in the near future. ColdPress requires the zmin of the PDF to be between -0.2 and 12.48. This is sufficient for any realistic PDF of an observed source.]]

[[NOTE: the truncation threshold is relative to the bin with the maximum integrated probability or  the point with maximum probability density.]]
