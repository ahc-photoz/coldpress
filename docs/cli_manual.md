...
## `check`
Analyzes input PDFs (binned or sampled) for non-finite values, delta-function-like properties, or truncation, and flags them. Running this command is a recommended practice to identify issues prior to compression.

Currently, `--density` is not explicitly implemented, but density-type PDFs can be evaluated using the `--binned` argument.

The `check` command does not currently verify if the input PDFs extend beyond the redshift range supported by ColdPress encoding. ColdPress requires the zmin of the PDF to fall between -0.2 and 12.48. This is sufficient for realistic observational PDFs, and out-of-bounds checks will be implemented in a future release.
...
* `--truncation-threshold THRESHOLD`: Probability threshold at grid edges to trigger a truncation flag. This threshold is relative to the bin with the maximum integrated probability or the point with the maximum probability density (default: 0.05).
...
## `measure`
Computes point-estimate statistics (e.g., mean, mode, credible intervals) directly from compressed PDFs. All measurements utilize linear interpolation of the cumulative distribution function (CDF) between consecutive quantiles.

The `--seed` argument ensures the reproducibility of `Z_RANDOM` values at the table level. Random values depend on the row index; therefore, two identical PDFs in the same table will yield different random values, and changing a PDF's row order will alter its `Z_RANDOM` value.

The `ODDS` parameters (`ODDS_MODE`, `ODDS_MEAN`, `ODDS_MEDIAN`) represent the integrated probability within a window centered on the corresponding point estimate. The window has a half-width equal to `ODDS_WINDOW * (1+z)`.
...
> **Tip:** Since version 1.2.0, you can plot multiple probability distributions for the same source in a single plot by providing multiple column names to the `--encoded` argument. This is particularly useful for visually verifying the results of the `combine` command.

The interactive mode (`--interactive`) enables panning and zooming for close inspection of the PDFs. 

The `--quantities` argument accepts any scalar column in the FITS table to overplot as vertical markers, not just those generated by `coldpress measure`. This is useful for diagnostic comparisons, such as overlaying spectroscopic redshifts.

> **Important:** Use caution when applying the `--plot-all` flag to large tables to avoid excessive I/O load.
